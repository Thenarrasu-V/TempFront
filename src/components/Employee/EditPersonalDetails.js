...
const EditProfileSettingsPage = () => {
      ...
      try {
        ...
        if (userData) {
          ...
          setProfile({
            ...
            hrId: userData.hr ? userData.hr.id : '', // Extract hr_id from hr object
            userRole: userData.role || '',
            managerId: userData.manager ? userData.manager.id : '', // Extract manager_id from manager object
          });
        } else {
  ...
  const postProfileSettings = async (updatedProfile, hrId, managerId) => {
    try {
      const response = await axios.post('http://localhost:8080/profileSettings/update', {
        ...updatedProfile,
        hr: { id: hrId },
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      ...
      const managerId = JSON.parse(localStorage.getItem('user'))?.managerId;

      try {
        await postProfileSettings(profile, hrId, managerId);
        setIsError(false);
